#!/usr/bin/env bash
# Build a2a-bridge image via AgentBuild CRD
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../lib/env-detect.sh"
source "$SCRIPT_DIR/../lib/logging.sh"

log_step "79" "Building a2a-bridge image"
...
kubectl apply -f "$REPO_ROOT/kagenti/examples/mcpservers/a2a_bridge_build.yaml"

# Wait for AgentBuild to exist
for i in {1..150}; do
    if kubectl get agentbuild a2a-bridge-build -n kagenti-agents &> /dev/null; then
        break
    fi
    if [ "$i" -eq 150 ]; then
...
for i in {1..90}; do
    phase=$(kubectl get agentbuild a2a-bridge-build -n kagenti-agents -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
    log_info "AgentBuild phase: $phase"
    if [ "$phase" = "Succeeded" ]; then
        log_success "a2a-bridge build completed successfully"
        exit 0
    elif [ "$phase" = "Failed" ]; then
        log_error "AgentBuild failed"
        kubectl describe agentbuild a2a-bridge-build -n kagenti-agents
