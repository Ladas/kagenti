#!/usr/bin/env bash
# Build k8s-debug-agent image via AgentBuild CRD
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../lib/env-detect.sh"
source "$SCRIPT_DIR/../lib/logging.sh"

log_step "77" "Building k8s-debug-agent image"
...
kubectl apply -f "$REPO_ROOT/kagenti/examples/agents/k8s_debug_agent_build.yaml"

# Wait for AgentBuild to exist
for i in {1..150}; do
    if kubectl get agentbuild k8s-debug-agent-build -n kagenti-agents &> /dev/null; then
        break
    fi
    if [ "$i" -eq 150 ]; then
...
for i in {1..90}; do
    phase=$(kubectl get agentbuild k8s-debug-agent-build -n kagenti-agents -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
    log_info "AgentBuild phase: $phase"
    if [ "$phase" = "Succeeded" ]; then
        log_success "k8s-debug-agent build completed successfully"
        exit 0
    elif [ "$phase" = "Failed" ]; then
        log_error "AgentBuild failed"
        kubectl describe agentbuild k8s-debug-agent-build -n kagenti-agents
